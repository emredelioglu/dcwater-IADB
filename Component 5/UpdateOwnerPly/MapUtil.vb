...
ErrorHandler:
    ...
    Public Shared Sub CopyFeature(ByVal fromFeature As IFeature, ByVal tofeature As IFeature, Optional ByVal bWithShape As Boolean = False)
        If bWithShape Then
            tofeature.Shape = fromFeature.ShapeCopy
        End If

        Dim pFields As IFields
        pFields = tofeature.Fields

        Dim i As Integer
        For i = 0 To pFields.FieldCount - 1
            If Not pFields.Field(i).Type = esriFieldType.esriFieldTypeOID _
                And Not pFields.Field(i).Type = esriFieldType.esriFieldTypeGeometry _
                And pFields.Field(i).Editable Then

                Dim fIndex As Integer = fromFeature.Fields.FindField(pFields.Field(i).Name)
                If fIndex >= 0 Then
                    tofeature.Value(i) = fromFeature.Value(fIndex)
                End If
    ...
    Public Shared Sub CopyRow(ByVal fromRow As IRow, ByVal toRow As IRow)
        Dim pFields As IFields
        pFields = toRow.Fields
        ...
        For i = 0 To pFields.FieldCount - 1
            If Not pFields.Field(i).Type = esriFieldType.esriFieldTypeOID _
                ...
                Dim fIndex As Integer = fromRow.Fields.FindField(pFields.Field(i).Name)
                If fIndex >= 0 Then
                    toRow.Value(i) = fromRow.Value(fIndex)
                End If
